...
$mydb = new DbConn();
$voting_stat = $mydb->selectQry("SELECT `event_name`,`end_date`,`end_time`,`event_img_path`,`voting_start`,`voting_end` from event_details");
$event_info = $mydb->selectcount($voting_stat);
function convertDate($date) //2020/10/29
{
...
function convertTime($time) //11:59:00 00 to 23
{
            ...
            <div class="vote-cards">
                <div class="vote-card">
                    <div class="vote-card-content">
                        <h2 class="vote-event-name sub-heading"><?php echo $event_name;  ?></h2>
                        <h2 class="deadline para-small-text">Deadline: <span><?php echo substr($end_date, 8, 2) . ' ' . convertDate($end_date) . ' ' . substr($end_date, 0, 4);  ?>, <?php echo convertTime($end_time);  ?> </span></h2>
                        <br>
                        <button class="btn btn-normal btn-small para-small-text white-text"><a href="">Vote now</a></button>
                    </div>
                    <div class="vote-event-image">
                        <img src="<?php echo $img_path;  ?>" alt="Event Image">
                    </div>
                </div>
            </div>
